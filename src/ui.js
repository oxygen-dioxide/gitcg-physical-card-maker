import { CARD_TYPE_LABELS, COST_DEFS, TAG_DEFS, HIDDEN_TAGS, assetsUrl } from './constants.js'
...
function downloadCard() {
  renderCard({
    ...
    a.download = (document.getElementById('name-input')?.value || 'card') + '.png'
...
function createCostRow(index, type, count) {
  ...
  Object.entries(COST_DEFS).forEach(([k, v]) => {
    sel.appendChild(el('option', { value: k }, v.label + ' ' + k.split('_').pop()))
  })
  if (type) sel.value = type
  const num = el('input', { type: 'number', min: '0', max: '10', value: String(count || 0), className: 'cost-count' })
...
function bindCostEvents() {
  ...
  document.querySelectorAll('#cost-list .cost-row input[type="number"]').forEach(s => {
    s.oninput = updatePreview
  })
  document.querySelectorAll('#cost-list .cost-row .btn-del').forEach(b => {
    b.onclick = function () { this.closest('.cost-row').remove(); updatePreview() }
  })
  document.querySelectorAll('#cost-list .cost-row .btn:not(.btn-del)').forEach(b => {
    b.onclick = function () {
...
function renderTagDisplay() {
  ...
  allTags.forEach((t, i) => {
    ...
    if (t.iconFile) {
      chip.appendChild(el('img', { src: assetsUrl(t.iconFile), style: 'width:14px;height:14px;' }))
    }
...
function selectCard(card) {
  ...
  ;(card.tags || []).forEach(t => {
    ...
    if (def && !HIDDEN_TAGS.has(t)) allTags.push({ key: t, label: def.label, iconFile: def.icon })
  })
  ...
  document.getElementById('effect-sub').value = parts.slice(1).join('\n').replace(/^（/, '').replace(/）$/, '')
...
function renderCardList(keyword) {
  ...
  shown.forEach(card => {
    ...
    const thumb = el('img', { src: imageUrlFor(card.id), className: 'card-thumb', loading: 'lazy' })
